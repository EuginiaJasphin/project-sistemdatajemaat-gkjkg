...
include_once("pagination_class.php");
   ...
   $nama = $_GET['nama'];
   $sql = "select * from tbl_pernikahan where pasangan like '%$nama%' order by id_jemaat DESC";}
   
   else if (isset($_GET['id']) and !empty($_GET['id'])){
   $id = $_GET['id'];
   $sql = "select * from tbl_pernikahan where pasangan= '$id' order by id_jemaat DESC";}
   
   else { $sql = "select * from tbl_pernikahan WHERE substr(pasangan,7,2) like'%$_SESSION[wilayah]%' order by id_pernikahan DESC";
...
<table id="tblpernikahan" width="860"><tr>
  <th width="181">Pasangan</th>
  <th width="122">Tanggal </th>
  <th width="121">di Gereja </th>
  <th>Oleh Pendeta </th>
  <th width="170">Aksi
       <? 
       ...
       { echo $nama['nama_jemaat']; }?></td><td><?= $r['tanggal']; ?></td>
       <td><?= $r['gereja']; ?></td>
       <td><?= $r['pendeta']; ?></td>
       <td>	<a href="formpernikahan.php?action=update&id=<?= $r['id_pernikahan'];?>" class="edit">edit</a> |
	    <a href="proses_data.php?action=del_nikahan&idnikahan=<?= $r['id_pernikahan'];?>" class="delete">delete</a>
		<?php } //end while ?></td></tr>
