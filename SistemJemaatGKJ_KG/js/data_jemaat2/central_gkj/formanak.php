...
<form method="post" name="formanak" action="" id="formanak">
  <table width="860">
    ...
    <tr>
      ...
      <td colspan="3"><select name="pasangan" id="pasangan">
	<? 
	   if (($_GET['action'] == "update")){
	   $result=mysql_query("select * from tbl_jemaat WHERE id_jemaat='$namaorangtua'");
       while($row = mysql_fetch_array($result)){
	   $pasangan=$row['id_jemaat'];
	   $rs=mysql_query("select * from tbl_jemaat WHERE pasangan='$pasangan' AND ket='$_SESSION[wilayah]' ORDER BY nama_jemaat ASC");
	 ...
	 ?>
      ...
      <td><b>Atribut data Anak-anak</b></td>
    </tr>
    <tr>
      <td>Hubungan Anak </td>
      <td colspan="3"><select name="hubungan">
      ...
      <td>o <a href="?adm=in_data&amp;status=pendidikan&amp;aksi=anak">data sekolah</a> >> </td>
    </tr>
    <tr>
      <td>Nama Lengkap Anak </td>
      <td colspan="3"><input type="text" id="namanak" name="namanak" size="30"  value="<?= $nama; ?>" /></td>
      <td>o <a href="?adm=in_data&amp;status=pekerjaan&amp;aksi=anak">data pekerjaan</a> >> </td>
    </tr>
    <tr>
...
<select name="tahun">
  ...
  <?php for ($i=1940; $i <= 2020; $i++) { if ($i == $outyear){ $selectyear ="selected";
    ...
    <tr>
      ...
      <td colspan="4"><select name="pendterakhir">
	...
	<? 
	   if (($_GET['action'] == "update")){
	   $rs=mysql_query("select * from tbl_jemaat WHERE id_jemaat='$idplgn'");
	   while($rows = mysql_fetch_array($rs)){
	   echo "<option value=$rows[a_gereja]>$rows[a_gereja]</option>";
    ...
    <tr>
      <td colspan="5"><input name="submit" type="submit" value="<?php echo $status; ?>" /> | <a href="../central_gkj/main.php?adm=in_data&status=anak"><input type="reset" name="reset" value="Tambah baru" /></a></td>
    </tr>
  </table>
