...
$(function(){ 
  ...
  $('#tblpekerjaan tr:odd:not(#nav):not(#total)').addClass('odd');
	...
	$("a.delete").click(function(){
		el=$(this);
		if(confirm("Apakah benar akan menghapus data pekerjaan?"))
		{
			$.ajax({
				url:$(this).attr("href"), 
				type:"GET",
...
<? if ($recor==0 AND $action=='update_kerja') { echo "Data Pekerjaan belum ditambahkan, Untuk menambahkan klik <a href='main.php?adm=in_data&status=pekerjaan&id=$_GET[id]'>
TAMBAH PEKERJAAN</a> ?"; } else { ?>
...
<tr><td colspan="6"><input type="submit" value="<?php echo $status; ?>" /> | <a href="../central_gkj/main.php?adm=in_data&status=pekerjaan"><input type="reset" name="reset" value="Tambah baru" /></a>
<? if($_GET['action']=='update') {?>| <a href="proses_data.php?action=del_kerjaan&id_pekerjaan=<?php echo $id_pekerjaan;?>" class="delete"><input type="reset" name="reset" value="Delete Data" /></a> <? } ?></td></tr>
