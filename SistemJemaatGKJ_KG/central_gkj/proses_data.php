<?php
  ...
  // respon balikan dari masing-masing proses tersebut adalah dalam format JSON.
	...
	$pernikahan=$_POST['pernikah'];$pasangan=$_POST['pasangan'];$pasangan2=$_POST['pasangan2'];$wil=$_POST['wil'];$kode=$_POST['kode'];$hub=$_POST['hub'];
...
//UNTUK DATA PRIBADI
	 ...
	 elseif($action=="add_anak") //menangani aksi penambahan data anak-anak
    {
	 ...
	 a_gereja,hobby,minat,m_pelayanan,ket,anak)        
	  VALUES('$idjemaat','$status','$pasangan','$namanak','$namapanggilan','$tempatlahir','$jkel','$tanggal','$bulan','$tahun','$goldarah','$alamat RT$rt RW$rw','$kelurahan','$kecamatan','$kota','$provinsi','$pendterakhir','$no_hp','$notelp','$sekolah','$kerja','$a_gereja','$hobby','$minat','$m_pelayanan','$ket','$hubungan')") 
	  or die ("data gagal ditambahakan!");
    // mengembalikan respon dalam format JSON.
    // status "1" berarti proses berhasil dilakukan.
    echo '{"status":"1"}';  
	...
	elseif($action=="add_babtis") //menangani aksi penambahan data Babtis, Atestasi, Sidi
    {
	 ...
	 $nosurat=$_POST['nosurat'];$gerejasidi=$_POST['gerejasidi'];$tanggalsidi=$_POST['tanggalsidi'];$pendetasidi=$_POST['pendetasidi'];
	 ...
	 gerejasidi,tanggalsidi,pendetasidi,nosuratsidi,gerejapenyerahan,tanggalpenyerahan,pendetapenyerahan)        
	...
	elseif($action=="add_gereja") //menangani aksi penambahan data usaha keluarga
    {
	 $nama=$_POST['nama'];$alamat=$_POST['alamat'];
	 
	 mysql_query("INSERT INTO tbl_gereja(nama,alamat)        
	 VALUES('$nama','$alamat')") or die ("data gagal ditambahakan!");
     echo "<script>alert('Data telah tersimpan');self.location.href='main.php?adm=in_data&status=gereja';</script>"; 
	 exit;
	}
...
$test = mysql_query("UPDATE tbl_pendidikan SET id_jemaat='$idjemaat',jenjangpend='$jenjangpend',namasekolah='$namasekolah',thnlulus='$thnlulus',tempat='$tempat',gelar='$gelar',pendkhusus='$pendkhusus' WHERE id_pendidikan='$id_pendidikan'") or die ("data gagal di-update!");
echo '{"status":"1"}';
exit;}
...
elseif($action=="update_babtis") //menangani aksi perubahan data babtis
{   
...
$test = mysql_query("UPDATE tbl_babtis SET gerejababtis='$gerejababtis',tanggalbabtis='$tanggalbabtis',nosuratbabtis='$nosuratbabtis',pendetababtis='$pendetababtis',gerejatestasi='$gerejatestasi',tanggalatestasi='$tanggalatestasi',nosurat='$nosurat',gerejasidi='$gerejasidi',tanggalsidi='$tanggalsidi',pendetasidi='$pendetasidi',nosuratsidi='$nosuratsidi',gerejapenyerahan='$gerejapenyerahan',tanggalpenyerahan='$tanggalpenyerahan',pendetapenyerahan='$pendetapenyerahan' WHERE id_jemaat='$idjemaat'") or die ("data gagal di-update!");
echo "<script>alert('Data telah tersimpan');self.location.href='main.php?adm=in_update';</script>";}
...
elseif($action=="update_gereja") //menangani aksi perubahan data usaha keluarga
{   
$nama=$_POST['nama'];$alamat=$_POST['alamat'];
$test = mysql_query("UPDATE tbl_gereja SET nama='$nama',alamat='$alamat' WHERE id_gereja='$_REQUEST[id_asli]'") or die ("data gagal di-update!");
echo "<script>alert('Data telah terupdate');self.location.href='main.php?adm=in_data&status=gereja';</script>"; 
exit;}
...
$test = mysql_query("delete from tbl_pekerjaan where id_pekerjaan='$id'");
if(mysql_affected_rows() == 1){ //jika jumlah baris data yang dikenai operasi delete == 1
echo '{"status":"1"}';}else{echo '{"status":"0"}';}exit;}
...
{$id = $_GET['idusaha'];
$test = mysql_query("delete from tbl_usaha where id_usaha='$id'");
if(mysql_affected_rows() == 1){ //jika jumlah baris data yang dikenai operasi delete == 1
echo '{"status":"1"}';}else{echo '{"status":"0"}';}exit;}
...
elseif($_GET['action']=="del_gereja") {
$id = $_GET['id_gereja'];
$test = mysql_query("delete from tbl_gereja where id_gereja='$id'") or die ("data gagal di-update!");
echo "<script>alert('Data telah terhapus');self.location.href='main.php?adm=in_data&status=gereja';</script>";
exit;}
?>