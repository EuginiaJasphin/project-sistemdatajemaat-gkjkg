...
class Pagination_class{
	...
	function Pagination_class($qry,$starting,$recpage)
	{
		...
		if($previous < 0){
			$anc .= "<li class='previous-off'>pertama</li>";
			$anc .= "<li class='previous-off'><<</li>";
		}else{
			$anc .= "<li class='next'><a href='javascript:pagination(0);'>pertama</a></li>";
		...
		if($next >= $numrows){
			$anc .= "<li class='previous-off'>>> </li>";
			$anc .= "<li class='previous-off'>terakhir</li>";
		}else{
			$anc .= "<li class='next'><a href='javascript:pagination($next);'>>> </a></li>";
			$anc .= "<li class='next'><a href='javascript:pagination($last);'>terakhir </a></li>";
		}
			$anc .= "</ul>";
		$this->anchors = $anc;
