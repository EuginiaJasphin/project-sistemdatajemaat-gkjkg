...
$action="add_pernikahan";
	...
	{
		$str="select * from tbl_pernikahan where pasangan = '$_GET[id]'";
...
$(function(){ 
  ...
  $('#tblpernikahan tr:odd:not(#nav):not(#total)').addClass('odd');
	...
	$("a.delete").click(function(){
		el=$(this);
		if(confirm("Apakah benar akan menghapus ?"))
		{
			$.ajax({
				url:$(this).attr("href"), 
				type:"GET",
...
<?php if ($recor==0 AND $action=='update_nikah') { echo "Data Pernikahan belum ditambahkan, Untuk menambahkan klik 
<a href='main.php?adm=in_data&status=pernikahan&actam=in&id=$id_dirijemaat'>
TAMBAH PERNIKAHAN</a> ?"; } else { ?>
<table width="860">
  <tr>
...
<?php }?>
<tr>
  <td width="177">Nama Kepala Keluarga </td>
...
<tr>
  ...
  <td><select name="gereja" id="gereja">
	<? 
	   if (($_GET['action'] == "update")){
	   $rs=mysql_query("select P.id_pernikahan,P.gereja,G.id_gereja,G.nama from tbl_pernikahan P, tbl_gereja G WHERE P.id_pernikahan='$idpernikahan' AND P.gereja=G.id_gereja
	   GROUP BY P.id_pernikahan");
...
<tr><td colspan="2"><input type="submit" value="<?php echo $status; ?>" /> | <a href="../central_gkj/main.php?adm=in_data&status=pernikahan"><input type="reset" name="reset" value="Tambah baru" /></a>
<? if($_GET['action']=='update') {?>| <a href="proses_data.php?action=del_nikahan&idnikahan=<?php echo $idpernikahan;?>" class="delete"><input type="reset" name="reset" value="Delete Data" /></a> <? } ?></td></tr>
</table>
<input type="hidden" name="action" value="<?php echo $action; ?>" />
</form></td>
   <td width="174" valign="top">
      <? if ($_GET['status'] == "pernikahan"){ ?>
	  <br />
      Atribut Data Pernikahan
      <hr color="#CCCCCC" width="75%" size="1" align="left"/>
	  <? include"atribut_gereja.php";}
	     else { echo "";} ?></td>
  </tr>
</table>
<? }?>