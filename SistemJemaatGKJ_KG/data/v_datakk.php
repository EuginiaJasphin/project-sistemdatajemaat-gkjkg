...
<body>
<b>Data Jemaat Berdasarkan Kepala Keluarga Tiap Wilayah</b>
...
$jmlperhalaman = 30;  // jumlah record per halaman
$offset = (($page * $jmlperhalaman) - $jmlperhalaman);
  
// membaca data yang ditampilkan
$wilayah=$_REQUEST['wil'];
$sql = mysql_query("SELECT * FROM tbl_jemaat WHERE (ket like '$_REQUEST[wil]%'  AND nama_jemaat like '%$_REQUEST[nama]%') AND anak='KK' ORDER BY id_jemaat ASC LIMIT $offset, $jmlperhalaman ");
$banyaknya = mysql_query("SELECT * FROM tbl_jemaat WHERE (ket like '$_REQUEST[wil]%' AND nama_jemaat like '%$_REQUEST[nama]%') AND anak='KK'"); 
...
while($row = mysql_fetch_array($sql))
 ...
 $pasangan= $row['pasangan'];$orangtua = $row['nama_orangtua']; ?></td>
  </tr>
  <tr>
    <td><? echo $id_jemaat; ?></td>
    <td><a href="?act=datarekap&id=<?
	if(($row['jkel']=='P') && ($row['pernikahan']=='menikah') && ($pasangan!='janda') && ($pasangan!='duda')) { echo $pasangan; }
	else if (($row['jkel']=='P') &&($row['pernikahan']=='')) { echo $id_jemaat; }
...
$total_record = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_jemaat WHERE (ket like '$_REQUEST[wil]%' AND nama_jemaat like '%$_REQUEST[nama]%') AND anak='KK'"),0);
$total_halaman = ceil($total_record / $jmlperhalaman);
$perhal=4;
if($hal > 1){ 
$prev = ($page - 1); 
echo "<a href=?act=dataKK&hal=$prev&wil=$wilayah> kembali </a> "; 
}
...
for($i = $hal1; $i <= $hal2; $i++){ 
    ...
    if($i<=$total_halaman){
            echo "<a href=?act=datakk&hal=$i&wil=$wilayah>$i</a> "; 
    }
    } 
}
if($hal < $total_halaman){ 
    $next = ($page + 1); 
    echo "<a href=?act=datakk&hal=$next&wil=$wilayah> lanjutkan </a>"; 
}  
?></div>
</form>
</body>
